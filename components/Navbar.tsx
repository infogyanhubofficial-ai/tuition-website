          ...
          <div className="flex items-center justify-between w-full h-full gap-2 xl:gap-4">
            ...
            <Link href="/" className="flex-shrink-0 outline-none rounded-lg focus-visible:ring-2 focus-visible:ring-[#2D9CDB] z-20">
              <motion.div 
                ...
                className={`relative transition-all duration-500 ${
                  scrolled ? 'w-[120px] h-[34px] lg:w-[140px] lg:h-[42px]' : 'w-[140px] h-[40px] lg:w-[170px] lg:h-[50px]'
                }`}
              >
                ...
                {loadingUser ? (
                  ...
                  <>
                    <button 
                      ...
                      className="group flex items-center gap-2 outline-none rounded-full focus-visible:ring-2 focus-visible:ring-[#2D9CDB] bg-white hover:bg-slate-50 border border-slate-100 pr-3 transition-all duration-200 shadow-sm whitespace-nowrap"
                    >
            ...
            <div className="flex items-center gap-2 ml-auto lg:hidden z-20">
              {loadingUser ? (
                ...
                <button onClick={handleLogin} className="text-[12px] font-black text-slate-700 px-4 py-2 bg-slate-100 border border-slate-200 rounded-full hover:bg-slate-200 transition-colors whitespace-nowrap">
                  LOGIN
                </button>
              ) : (
                <div className="relative h-9 w-9 rounded-full overflow-hidden border border-slate-200 shadow-sm cursor-pointer" onClick={() => setMobileOpen(true)}>
                  <Image src={user?.user_metadata?.avatar_url || "/images/default-avatar.png"} alt="Profile" fill sizes="36px" className="object-cover" />
                </div>
              )}
              <button 
                onClick={() => setMobileOpen(true)} 
                aria-label="Open Mobile Menu"
                className="p-2 rounded-full text-slate-700 bg-slate-50 border border-slate-200 hover:bg-slate-100 transition-colors flex items-center justify-center"
              >
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-[100] flex flex-col">
            ...
            <motion.div 
              ...
              className="relative w-full bg-white bg-gradient-to-b from-blue-50/50 to-white rounded-b-[2.5rem] p-6 sm:p-8 shadow-2xl flex flex-col max-h-[90vh] overflow-y-auto"
            >
              <div className="flex justify-between items-center mb-6">
                <Image src="https://zuktarghyexwodqnnxlu.supabase.co/storage/v1/object/public/others/GyanHub_logo_website-removebg-preview.png" alt="Logo" width={120} height={40} />
                <button 
                  onClick={() => setMobileOpen(false)} 
                  className="p-2.5 bg-slate-100 rounded-full text-slate-700 hover:bg-slate-200 transition-colors flex items-center justify-center"
                >
              ...
              {!loadingUser && user && (
                <div className="bg-gradient-to-br from-[#2D9CDB]/10 to-[#FF6B35]/10 p-5 rounded-3xl mb-8 border border-white shadow-sm flex flex-col items-center text-center">
                  <div className="relative w-20 h-20 rounded-full overflow-hidden border-4 border-white shadow-md mb-3">
                    <Image src={user?.user_metadata?.avatar_url || "/images/default-avatar.png"} alt="User" fill sizes="80px" className="object-cover" />
                  </div>
                  <p className="text-[10px] font-black uppercase tracking-widest text-[#FF6B35] mb-1">Welcome Back!</p>
                  <h3 className="text-lg font-black text-slate-900 mb-4">{user?.user_metadata?.full_name || 'My Profile'}</h3>
                  <Link href="/profile" onClick={() => setMobileOpen(false)} className="w-full bg-white text-[#2D9CDB] py-3 rounded-2xl font-black text-sm shadow-sm hover:shadow-md transition-all border border-blue-100">
                    Go to Dashboard
                  </Link>
                </div>
              )}

              <motion.nav variants={mobileLinkContainerVars} className="flex flex-col gap-2 mb-6">
                {navItems.map((item) => {
                  const active = isActive(item.href);
                  return (
                    <motion.div variants={mobileLinkVars} key={item.href}>
                      <Link 
                        href={item.href} 
                        onClick={() => setMobileOpen(false)} 
                        className={`flex items-center gap-4 px-5 py-4 min-h-[52px] rounded-2xl text-[17px] font-black transition-all duration-200 border ${
                          active ? "bg-[#2D9CDB]/10 text-[#2D9CDB] border-[#2D9CDB]/20 shadow-sm" : "bg-white text-slate-700 border-slate-100 hover:bg-slate-50 hover:scale-[1.02]"
                        }`}
                      >
              ...
              <div className="mt-auto sticky bottom-0 bg-white/95 backdrop-blur-md pt-4 pb-2 border-t border-slate-100 flex flex-col gap-3">
                {!loadingUser && !user && (
                  <button onClick={handleLogin} className="w-full flex items-center justify-center rounded-2xl bg-slate-100 min-h-[56px] text-[15px] font-black text-slate-800 transition-colors hover:bg-slate-200">
                    Log In / Sign Up
                  </button>
                )}

                {!loadingUser && !isTutor && (
                   <Link href="/become-a-tutor" onClick={() => setMobileOpen(false)} className="w-full flex items-center justify-center gap-2 rounded-2xl border-2 border-slate-200 text-slate-700 hover:border-[#2D9CDB] hover:text-[#2D9CDB] min-h-[56px] font-black transition-all active:scale-95 whitespace-nowrap">
                     <UserPlus className="h-5 w-5" strokeWidth={2.5} /> Become a Tutor
                   </Link>
                )}
                
                <Link href="/online-courses" onClick={() => setMobileOpen(false)} className="w-full flex items-center justify-center gap-2 rounded-2xl bg-[#FF6B35] text-white min-h-[56px] font-black shadow-[0_8px_20px_rgba(255,107,53,0.3)] transition-all active:scale-95 whitespace-nowrap">
                  <MonitorPlay className="h-5 w-5" strokeWidth={2.5} /> Explore Online Classes
                </Link>

                {!loadingUser && user && (
                  <button onClick={handleLogout} className="mt-2 w-full text-center rounded-2xl bg-red-50/50 py-3 text-sm font-bold text-red-600 transition-colors hover:bg-red-100">
                    Log Out
                  </button>
                )}
              </div>
