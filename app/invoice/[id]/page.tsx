...
export default function SingleInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // This ensures the generated PDF always uses the desktop layout even if downloaded from mobile
  ...
  return (
    <div className="fixed inset-0 z-[9999] overflow-y-auto bg-gray-100 py-6 md:py-10 font-sans text-gray-800 print:relative print:inset-auto print:overflow-visible print:py-0 print:bg-white print:z-auto print:h-auto flex flex-col items-center">
      
      {/* ==========================================
          WEB CONTROLS (Fully Responsive)
          ========================================== */}
      <div className="w-full max-w-[210mm] mb-4 flex flex-col sm:flex-row justify-between items-center gap-3 print:hidden px-4 md:px-0">
        ...
        <div className="flex w-full sm:w-auto gap-3">
          <button
            ...
            className="flex-1 sm:flex-none bg-white border border-[#0A2A66] text-[#0A2A66] hover:bg-gray-50 px-6 py-2.5 rounded shadow-sm transition-all font-medium text-sm sm:text-base"
          >
            Print
          </button>
          <button
            ...
            className="flex-1 sm:flex-none bg-[#0A2A66] hover:bg-blue-800 text-white px-6 py-2.5 rounded shadow-md transition-all flex justify-center items-center gap-2 font-medium disabled:opacity-70 text-sm sm:text-base"
          >
            {isDownloading ? (
      ...
      {/* ==========================================
          ...
          Dynamically scales to fit mobile via Tailwind width classes, caps at A4 size.
          ========================================== */}
            ...
            <div className="flex flex-col lg:flex-row mt-6 sm:mt-8 gap-6 md:gap-4">
              ...
              <div className="w-full lg:w-5/12">
                ...
                {order.status !== 'verified' && (
                  <div className="mt-4 sm:mt-6 border-[3px] sm:border-[4px] border-red-600 border-dashed text-red-600 font-black text-[9px] sm:text-[11px] leading-tight uppercase tracking-widest p-3 sm:p-4 transform -rotate-3 text-center bg-white mix-blend-multiply opacity-90">
                    <span className="block text-lg sm:text-xl mb-1">🚫 INVALID 🚫</span>
                    Invoice & Service<br/>Not Verified Yet
                  </div>
                )}
              </div>
            </div>
            
            {/* MOBILE ONLY THANK YOU NOTE (Displayed here for better flow on small screens) */}
