...
import { createClient } from "@/lib/supabase/client"; // <-- FIX 1: Correct client import
...
function GovernmentIdModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md dark:bg-black/60">
      <motion.div 
      ...
      >
        <button onClick={onClose} className="absolute right-6 top-6 flex h-8 w-8 items-center justify-center rounded-full bg-slate-50 dark:bg-slate-800 text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
          <X className="h-4 w-4" />
        </button>
        ...
        <h3 className="text-2xl font-black text-slate-900 dark:text-white leading-tight">Identity Verification</h3>
...
function AbstractHeroPattern() {
  return (
    <div className="absolute inset-0 overflow-hidden rounded-[40px] bg-white dark:bg-slate-950">
...
export default function TutorProfilePage() {
  ...
  const id = Array.isArray(rawId) ? rawId[0] : rawId; // Ensure string format
  
  const router = useRouter();
  const { scrollY } = useScroll();
  const supabase = createClient(); // <-- Initialize client properly
  ...
  // FIX 2: Bulletproof redirect handler
  const handleLockedAction = async (requestType: 'cv' | 'phone') => {
    triggerHaptic();
    
    // Fallback manual check just in case state is stale
    const { data: { session: currentSession } } = await supabase.auth.getSession();
    
    if (!currentSession) {
      router.push(`/login?redirect=/tutors/${id}`);
    } else {
      // Safely grab details using optional chaining to prevent silent crashes
      const userName = currentSession.user?.user_metadata?.full_name || '';
      const userEmail = currentSession.user?.email || '';
      const tutorName = tutor?.name || '';
      
      // Pass the tutor_name directly so the Order page doesn't have to fetch it!
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 transition-colors duration-300 pb-28 lg:pb-20 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-blue-600/20 [&::-webkit-scrollbar-thumb]:rounded-full">
            ...
            <div className="mx-auto max-w-7xl px-4 h-20 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="relative h-12 w-12 rounded-full overflow-hidden border-2 border-slate-100 dark:border-slate-800">
                  {avatarUrl ? (
                    <Image src={avatarUrl} alt={tutor.name || ''} fill className="object-cover" sizes="48px" unoptimized quality={100} />
                  ) : (
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <nav className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">
            ...
            <ChevronRight className="h-3 w-3" />
            <span className="text-slate-900 dark:text-white truncate max-w-[120px] sm:max-w-none">{tutor.name}</span>
          </nav>
                ...
                <div className="relative shrink-0 group z-10">
                  <div className="h-40 w-40 sm:h-48 sm:w-48 rounded-[36px] p-1 bg-gradient-to-tr from-blue-500 to-indigo-600 shadow-xl shadow-blue-500/20 dark:shadow-blue-900/40">
                    <div className="relative h-full w-full rounded-[32px] overflow-hidden border-4 border-white dark:border-slate-900 bg-slate-100 dark:bg-slate-800">
                      {avatarUrl ? (
                        <Image src={avatarUrl} alt={tutor.name || ''} fill className="object-cover relative z-10" sizes="(max-width: 768px) 160px, 192px" priority unoptimized quality={100} />
                      ) : (
                ...
                <div className="pb-2 flex-1 relative z-10">
                  <div className="flex flex-col justify-start mb-1">
                    <h1 className="text-3xl font-black sm:text-4xl tracking-tight text-slate-900 dark:text-white whitespace-normal break-words flex items-center gap-2">
                      {tutor.name}
                      {tutor.verified && (
                        <BadgeCheck className="h-7 w-7 text-blue-500 fill-blue-500 stroke-white dark:stroke-slate-900" />
                      )}
                    </h1>
                    {tutor.verified && (
                    ...
                    <div className="pt-0.5 overflow-hidden">
                      ...
                      <div className="relative flex items-center h-7 w-full overflow-hidden">
                        {isPremiumDataUnlocked ? (
                          ...
                          <div className="flex items-center gap-2">
                            <p className="text-base font-black tracking-wider text-slate-800 dark:text-slate-200 opacity-50 select-none">
