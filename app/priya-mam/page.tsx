...
type LeadStatus = "new" | "old" | "contacted" | "follow_up" | "interested" | "not_interested" | "enrolled";
...
interface CrmLead {
  ...
  lead_status: LeadStatus | null; payment_status: PaymentStatus | null; learning_mode: LearningMode | null;
  priority: Priority | null; call_notes: CallNote[] | null; last_contacted_at: string | null;
  follow_up_date: string | null; updated_at: string | null; created_at: string | null;
  interested_course_ids: number[] | null; syllabi_v2: SyllabusRef | null;
}
...
interface DashboardStats {
  totalLeads: number; newLeads: number; contactedToday: number; followupsToday: number;
  overdue: number; enrolledToday: number; highPriority: number;
}
...
interface ChipOption<T extends string> { value: T; label: string; dot: string; tone: string; }
...
function cnBase(...parts: string[]) { return parts.join(" "); }
...
function formatFollowUp(iso: string | null | undefined): { label: string; overdue: boolean } {
  ...
  const d = new Date(iso); if (Number.isNaN(d.getTime())) return { label: "Not set", overdue: false };
  const target = new Date(d); target.setHours(0, 0, 0, 0);
  const today = new Date(); today.setHours(0, 0, 0, 0);
  const diffDays = Math.round((target.getTime() - today.getTime()) / 86400000);
  let label = diffDays === 0 ? "Today" : diffDays === 1 ? "Tomorrow" : diffDays === -1 ? "1 day ago" : diffDays > 0 ? `In ${diffDays} days` : `${Math.abs(diffDays)} days ago`;
...
function dayLabel(iso: string): string {
  const diffDays = Math.round((new Date().setHours(0, 0, 0, 0) - new Date(iso).setHours(0, 0, 0, 0)) / 86400000);
  return diffDays === 0 ? "Today" : diffDays === 1 ? "Yesterday" : new Date(iso).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" });
}
...
function multiSelectLabel(selectedIds: number[], courses: SyllabusRef[], placeholder: string): string {
  ...
  const names = selectedIds.map((id) => courses.find((c) => c.id === id)?.name).filter(Boolean);
...
function AnimatedNumber({ value }: { value: number }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      setDisplay(Math.round(start + (value - start) * (1 - Math.pow(1 - t, 3))));
...
function KpiCard({ label, value, icon, accent, onClick, active }: { label: string; value: number; icon: React.ReactNode; accent: string; onClick?: () => void; active?: boolean; }) {
  const Tag = onClick ? "button" : "div";
  return (
    <Tag onClick={onClick} title={onClick ? `View ${label.toLowerCase()}` : undefined} className={cn("group flex items-center gap-3 rounded-[var(--radius-card)] border px-3.5 py-2.5 text-left transition-all duration-150", onClick && "cursor-pointer hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[0.07]", active ? "border-white/30 bg-white/[0.09] shadow-[0_8px_20px_-8px_rgba(0,0,0,0.5)]" : "border-white/10 bg-white/[0.04]")}>
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-control)] transition-transform duration-150 group-hover:scale-110" style={{ background: `linear-gradient(135deg, ${accent}33, ${accent}14)`, color: accent }}>{icon}</span>
      <div className="min-w-0">
        <p className={cn(TX.emph, "font-mono font-semibold leading-none text-white")}><AnimatedNumber value={value} /></p>
...
function CourseMultiSelect({ courses, selected, onChange, placeholder, addNewToFront = false }: { courses: SyllabusRef[]; selected: number[]; onChange: (ids: number[]) => void; placeholder: string; addNewToFront?: boolean; }) {
  ...
  const toggle = (id: number) => { onChange(selected.includes(id) ? selected.filter((s) => s !== id) : addNewToFront ? [id, ...selected] : [...selected, id]); };
  return (
    <div className="relative w-full" ref={ref}>
      <button onClick={() => setOpen(!open)} className={cn("flex h-8 w-full items-center gap-2 rounded-[var(--radius-control)] border px-2.5 bg-white transition-colors duration-150", TX.dense, "font-medium", selected.length > 0 ? "border-[var(--c-blue)] text-[var(--c-blue)] bg-[var(--c-blue-soft)]" : "border-slate-300 text-slate-500 hover:border-slate-400")}>
        <span className="truncate">{multiSelectLabel(selected, courses, placeholder)}</span>
        <ChevronDown size={ICON.sm} className={cn("ml-auto shrink-0 transition-transform duration-150", open && "rotate-180")} />
      </button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={MOTION} className="absolute left-0 z-30 mt-1.5 max-h-72 w-64 overflow-y-auto rounded-[var(--radius-control)] border border-slate-200 bg-white p-1.5 shadow-lg">
            {courses.length === 0 && <p className={cn(TX.body, "px-2 py-3 text-center text-slate-400")}>No courses found</p>}
            {courses.map((c) => {
              const active = selected.includes(c.id);
              return (
                <button key={c.id} onClick={() => toggle(c.id)} className={cn("flex w-full items-center gap-2 rounded px-2 py-1.5 text-left hover:bg-slate-50", TX.body, "text-slate-700")}>
                  <span className={cn("flex h-4 w-4 shrink-0 items-center justify-center rounded border", active ? "border-[var(--c-blue)] bg-[var(--c-blue)]" : "border-slate-300")}>{active && <Check size={11} className="text-white" strokeWidth={3} />}</span>
                  <span className="truncate">{c.name}</span>
                </button>
              );
            })}
            {selected.length > 0 && <button onClick={() => onChange([])} className={cn(TX.dense, "mt-1 w-full rounded px-2 py-1.5 text-left font-medium text-slate-400 hover:bg-slate-50 hover:text-slate-600")}>Clear selection</button>}
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function PillSelect<T extends string>({ label, value, options, onChange }: { label: string; value: T | "all"; options: { value: T; label: string }[]; onChange: (v: T | "all") => void; }) {
  return (
    <div className="relative">
      <select value={value} onChange={(e) => onChange(e.target.value as T | "all")} className={cn("h-9 cursor-pointer appearance-none rounded-[var(--radius-control)] border bg-white pl-3 pr-8 outline-none transition-colors duration-150", TX.body, "font-medium", value !== "all" ? "border-[var(--c-blue)] bg-[var(--c-blue-soft)] text-[var(--c-blue)]" : "border-slate-300 text-slate-600 hover:border-slate-400")}>
...
function ChipSelect<T extends string>({ value, options, onChange, disabled }: { value: T; options: ChipOption<T>[] ; onChange: (v: T) => void; disabled?: boolean; }) {
  ...
  return (
    <div className="relative w-full" ref={ref}>
      <button onClick={() => !disabled && setOpen(!open)} disabled={disabled} className={cn("flex h-8 w-full min-w-0 items-center gap-1.5 rounded-[var(--radius-control)] px-2 font-medium transition-opacity duration-150 disabled:opacity-50", TX.dense, current.tone)}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={MOTION} className="absolute left-0 z-30 mt-1.5 w-44 overflow-hidden rounded-[var(--radius-control)] border border-slate-200 bg-white p-1 shadow-lg">
            {options.map((o) => (
              <button key={o.value} onClick={() => { onChange(o.value); setOpen(false); }} className={cn("flex w-full items-center gap-2 rounded px-2 py-1.5 text-left font-medium hover:bg-slate-50", TX.dense, o.value === value ? "text-slate-900" : "text-slate-600")}>
...
function CoursesModal({ courses, onClose }: { courses: SyllabusRef[]; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }} transition={MOTION} className="flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden rounded-[var(--radius-card)] bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-5 py-4">
          ...
          <button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-[var(--radius-control)] text-slate-400 hover:bg-slate-200"><X size={ICON.md} /></button>
        </div>
        <div className="flex-1 space-y-2 overflow-y-auto bg-slate-50/50 p-4">
          {courses.map((c) => (
            <div key={c.id} className="overflow-hidden rounded-[var(--radius-control)] border border-slate-200 bg-white">
              <button onClick={() => setExpandedId(expandedId === c.id ? null : c.id)} className="flex w-full items-center justify-between px-4 py-3.5 text-left hover:bg-slate-50/80">
                <span className={cn(TX.body, "font-medium text-slate-800")}>{c.name}</span>
                <ChevronDown size={ICON.md} className={cn("text-slate-400", expandedId === c.id && "rotate-180")} />
              </button>
              <AnimatePresence>
                {expandedId === c.id && (
...
function LeadDrawer({ lead, onClose, onAddNote }: { lead: CrmLead; onClose: () => void; onAddNote: (leadId: number, note: string) => Promise<void> }) {
  ...
  return (
    <>
      ...
      <motion.aside initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 320, damping: 32 }} className="fixed right-0 top-0 z-50 flex h-screen w-full max-w-[440px] flex-col border-l border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center gap-3 border-b border-slate-200 bg-[var(--c-navy)] px-5 py-4">
          ...
          <button onClick={onClose} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-control)] text-white/70 hover:bg-white/10 hover:text-white"><X size={ICON.md} /></button>
        </div>
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
          <p className={cn(TX.micro, "mb-3 flex items-center gap-1.5 font-mono uppercase tracking-wider text-slate-400")}><History size={ICON.sm} /> Timeline · {(lead.call_notes ?? []).length} entries</p>
                ...
                <div className="space-y-1.5">
                  {g.notes.map((n) => (
                    <div key={n.id} className="relative rounded-[var(--radius-control)] bg-slate-50 px-3 py-2 pl-4">
                      ...
                      <p className={cn(TX.micro, "mt-1 font-mono text-slate-400")}>{formatTimestamp(n.created_at)}</p>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="border-t border-slate-200 bg-slate-50/70 px-5 py-4">
          ...
          <textarea value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="What happened on the call…" rows={2} className={cn(INPUT_CLS, TX.body, "w-full h-auto resize-none py-2 text-slate-700")} />
          <button onClick={async () => { if(!draft.trim()) return; setSaving(true); await onAddNote(lead.id, draft.trim()); setDraft(""); setSaving(false); }} disabled={!draft.trim() || saving} className={cn(BTN_PRIMARY_CLS, TX.body, "mt-2 h-8 w-full")}>
            {saving ? <Loader2 size={ICON.sm} className="animate-spin" /> : <MessageSquarePlus size={ICON.sm} />} Log note
          </button>
        </div>
      </motion.aside>
    </>
...
function SkeletonRow() {
  return (
    <tr className="border-b border-slate-100">
      <td className="px-2 py-3"><div className="h-3 w-5 animate-pulse rounded bg-slate-100" /></td>
      <td className="px-2 py-3"><div className="flex items-center gap-2.5"><div className="h-8 w-8 shrink-0 animate-pulse rounded-full bg-slate-100" /><div className="space-y-1.5"><div className="h-3 w-28 animate-pulse rounded bg-slate-100" /><div className="h-2.5 w-16 animate-pulse rounded bg-slate-100" /></div></div></td>
...
export default function CrmLeadsDashboard() {
  ...
  const [stats, setStats] = useState<DashboardStats>({ totalLeads: 0, newLeads: 0, contactedToday: 0, followupsToday: 0, overdue: 0, enrolledToday: 0, highPriority: 0 });
  ...
  const markPending = (id: number, on: boolean) => setPendingIds((prev) => { const next = new Set(prev); on ? next.add(id) : next.delete(id); return next; });
  const flash = (id: number) => { setFlashIds((prev) => new Set(prev).add(id)); setTimeout(() => { setFlashIds((prev) => { const next = new Set(prev); next.delete(id); return next; }); }, 900); };
  ...
  const fetchStats = useCallback(async () => {
    ...
    const [totalRes, newLeadsRes, contactedRes, followupsRes, overdueRes, enrolledRes, highPriorityRes] = await Promise.all([
      buildFilteredCountQuery(false), buildFilteredCountQuery(true).eq("lead_status", "new"),
      buildFilteredCountQuery(true).gte("last_contacted_at", startToday), buildFilteredCountQuery(true).eq("follow_up_date", todayStr),
      buildFilteredCountQuery(true).lt("follow_up_date", todayStr), buildFilteredCountQuery(true).eq("lead_status", "enrolled").gte("updated_at", startToday),
      buildFilteredCountQuery(true).eq("priority", "high"),
    ]);
    setStats({ totalLeads: totalRes.count ?? 0, newLeads: newLeadsRes.count ?? 0, contactedToday: contactedRes.count ?? 0, followupsToday: followupsRes.count ?? 0, overdue: overdueRes.count ?? 0, enrolledToday: enrolledRes.count ?? 0, highPriority: highPriorityRes.count ?? 0 });
  }, [buildFilteredCountQuery]);

  const refreshAll = useCallback(() => { fetchLeads(); fetchStats(); }, [fetchLeads, fetchStats]);
  ...
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) { if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === "k") { e.preventDefault(); searchInputRef.current?.focus(); } }
  ...
  const goToPage = (p: number) => { if (p < 1 || p > totalPages || p === page) return; setPage(p); };
  const courseMap = useMemo(() => { const m = new Map<number, SyllabusRef>(); courses.forEach((c) => m.set(c.id, c)); return m; }, [courses]);

  // Mutations
  const updateLeadStatus = async (id: number, lead_status: LeadStatus) => { const prev = leads; setLeads((cur) => cur.map((l) => (l.id === id ? { ...l, lead_status } : l))); markPending(id, true); const { error } = await supabase.from("crm_leads").update({ lead_status }).eq("id", id); markPending(id, false); if (error) { setLeads(prev); return; } flash(id); fetchStats(); };
  const updateLeadPriority = async (id: number, priority: Priority) => { const prev = leads; setLeads((cur) => cur.map((l) => (l.id === id ? { ...l, priority } : l))); markPending(id, true); const { error } = await supabase.from("crm_leads").update({ priority }).eq("id", id); markPending(id, false); if (error) { setLeads(prev); return; } flash(id); };
  const updateLearningMode = async (id: number, learning_mode: LearningMode) => { const prev = leads; setLeads((cur) => cur.map((l) => (l.id === id ? { ...l, learning_mode } : l))); markPending(id, true); const { error } = await supabase.from("crm_leads").update({ learning_mode }).eq("id", id); markPending(id, false); if (error) { setLeads(prev); return; } flash(id); };
  const updateFollowUpDate = async (id: number, dateValue: string) => { const follow_up_date = dateValue === "" ? null : dateValue; const prev = leads; setLeads((cur) => cur.map((l) => (l.id === id ? { ...l, follow_up_date } : l))); markPending(id, true); const { error } = await supabase.from("crm_leads").update({ follow_up_date }).eq("id", id); markPending(id, false); if (error) { setLeads(prev); return; } flash(id); fetchStats(); };
  const updateInterestedCourses = async (id: number, ids: number[]) => { const prev = leads; setLeads((cur) => cur.map((l) => (l.id === id ? { ...l, interested_course_ids: ids } : l))); markPending(id, true); const { error } = await supabase.from("crm_leads").update({ interested_course_ids: ids }).eq("id", id); markPending(id, false); if (error) { setLeads(prev); return; } flash(id); };

  const addCallNote = async (id: number, note: string) => { const lead = leads.find((l) => l.id === id); if (!lead) return; const newNote: CallNote = { id: crypto.randomUUID(), note, created_at: new Date().toISOString() }; const nextNotes = [...(lead.call_notes ?? []), newNote]; const prev = leads; setLeads((cur) => cur.map((l) => (l.id === id ? { ...l, call_notes: nextNotes } : l))); const { error } = await supabase.from("crm_leads").update({ call_notes: nextNotes }).eq("id", id); if (error) setLeads(prev); };
  const updateCallDraft = (id: number, value: string) => setCallDrafts((prev) => ({ ...prev, [id]: value }));

  const submitCallLog = async (id: number) => {
    const note = (callDrafts[id] ?? "").trim(); if (!note) return;
    ...
    const nextNotes = [...(lead.call_notes ?? []), newNote]; const now = new Date().toISOString();
    const shouldMarkContacted = (lead.lead_status ?? "new") === "new"; const nextStatus: LeadStatus = shouldMarkContacted ? "contacted" : lead.lead_status ?? "new";
    const prev = leads; setLeads((cur) => cur.map((l) => (l.id === id ? { ...l, call_notes: nextNotes, last_contacted_at: now, lead_status: nextStatus } : l))); setCallDrafts((d) => ({ ...d, [id]: "" })); markPending(id, true);
    const updatePayload: Partial<Pick<CrmLead, "call_notes" | "last_contacted_at" | "lead_status">> = { call_notes: nextNotes, last_contacted_at: now }; if (shouldMarkContacted) updatePayload.lead_status = "contacted";
    const { error } = await supabase.from("crm_leads").update(updatePayload).eq("id", id); markPending(id, false); if (error) { setLeads(prev); return; } flash(id); fetchStats();
  };

  const handleCopyPhone = (e: React.MouseEvent, raw: string | null) => {
    e.stopPropagation(); if (!raw) return; let digits = raw.replace(/\D/g, "");
    if (digits.length === 10) { digits = "+977" + digits; } else if (digits.startsWith("977") && digits.length > 3) { digits = "+" + digits; } else { digits = "+977" + digits; }
  ...
  const visibleLeads = useMemo(() => {
    ...
    return indexed.filter(({ lead: l }) => {
      const safeName = (l.name ?? "").toLowerCase(); const safePhone = (l.phone_number ?? "").replace(/\D/g, ""); const safeCourse = (l.syllabi_v2?.name ?? "").toLowerCase();
      return safeName.includes(q) || safePhone.includes(q.replace(/\D/g, "")) || safeCourse.includes(q);
    });
  }, [leads, search]);
  ...
  const rangeStart = totalCount === 0 ? 0 : (page - 1) * PAGE_SIZE + 1; const rangeEnd = Math.min(page * PAGE_SIZE, totalCount); const pageNumbers = getPageNumbers(page, totalPages);
  const drawerLead = drawerLeadId != null ? leads.find((l) => l.id === drawerLeadId) ?? null : null;

  if (!isUnlocked) {
    return (
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-[var(--c-navy)] font-[Inter,ui-sans-serif,system-ui] text-white">
        <style jsx global>{` ${TOKENS_STYLE} body { overflow: hidden; } `}</style>
        <div className="pointer-events-none absolute -left-10 top-0 h-56 w-56 rounded-full opacity-20 blur-3xl" style={{ background: "radial-gradient(circle, var(--c-accent), transparent 70%)" }} />
        <div className="pointer-events-none absolute -right-16 bottom-0 h-64 w-64 rounded-full opacity-20 blur-3xl" style={{ background: "radial-gradient(circle, #38BDF8, transparent 70%)" }} />
        
        <div className="relative flex w-full max-w-[360px] flex-col items-center rounded-2xl border border-white/10 bg-white/[0.04] p-8 shadow-2xl backdrop-blur-xl">
          <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-[var(--radius-control)] bg-[var(--c-accent)]/10 text-[var(--c-accent)] ring-1 ring-inset ring-[var(--c-accent)]/20"><Lock size={32} /></div>
          <h1 className={cn(TX.title, "mb-1.5 font-[Space_Grotesk,ui-sans-serif] font-semibold text-white")}>Restricted Access</h1>
          <p className={cn(TX.body, "mb-8 text-center text-slate-400")}>Enter your PIN to access the CRM dashboard.</p>
          <input type="password" autoFocus maxLength={6} value={pinInput} onChange={(e) => { const val = e.target.value.replace(/\D/g, ""); setPinInput(val); if (val === "112233") setIsUnlocked(true); }} className="h-12 w-full rounded-[var(--radius-control)] border border-white/20 bg-black/20 px-4 text-center text-2xl tracking-[0.5em] text-white outline-none transition-all placeholder:text-white/20 focus:border-[var(--c-accent)] focus:ring-2 focus:ring-[var(--c-accent)]/50" placeholder="••••••" />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="crm-scroll-shell fixed inset-0 z-[999] flex h-screen w-screen flex-col overflow-hidden bg-[#F7F8FA] font-[Inter,ui-sans-serif,system-ui]">
      <style jsx global>{`
        ...
        body > header:not(.crm-shell-header), body > nav, body > footer:not(.crm-shell-header), [data-site-chrome], #site-navbar, #site-footer, .site-header, .site-footer { display: none !important; }
        .crm-scroll-shell { -webkit-font-smoothing: antialiased; text-rendering: optimizeLegibility; }
        .crm-scroll-shell ::-webkit-scrollbar { width: 9px; height: 9px; }
        .crm-scroll-shell ::-webkit-scrollbar-track { background: transparent; }
        .crm-scroll-shell ::-webkit-scrollbar-thumb { background-color: rgba(30, 58, 138, 0.22); border-radius: 9999px; border: 2px solid transparent; background-clip: content-box; }
        .crm-scroll-shell ::-webkit-scrollbar-thumb:hover { background-color: rgba(30, 58, 138, 0.4); background-clip: content-box; }
      `}</style>

      {/* Header */}
      <header className="crm-shell-header relative shrink-0 overflow-hidden border-b border-slate-200 bg-[var(--c-navy)]">
        <svg className="pointer-events-none absolute inset-0 h-full w-full opacity-[0.06]" aria-hidden><defs><pattern id="bp-grid" width="28" height="28" patternUnits="userSpaceOnUse"><path d="M28 0H0V28" fill="none" stroke="white" strokeWidth="0.75" /></pattern></defs><rect width="100%" height="100%" fill="url(#bp-grid)" /></svg>
        ...
        <div className="relative mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-4 px-6 py-5">
          <div>
            <p className={cn(TX.micro, "flex items-center gap-1.5 font-mono uppercase tracking-[0.14em] text-[var(--c-accent)]")}><Sparkles size={ICON.sm} /> GyanHub · Lead Handler</p>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              {quickFilter && (
                <span className={cn(TX.body, "flex h-9 items-center gap-1.5 rounded-[var(--radius-control)] bg-[var(--c-blue-soft)] px-3 font-medium text-[var(--c-blue)] ring-1 ring-inset ring-[var(--c-blue-ring)]")}>
                  <Sparkles size={ICON.sm} />{QUICK_FILTER_LABELS[quickFilter]}
                  <button onClick={() => setQuickFilter(null)} className="ml-0.5 rounded-full p-0.5 transition-colors duration-150 hover:bg-[var(--c-blue)]/10" title="Clear quick filter"><X size={ICON.sm} /></button>
                </span>
              )}
        ...
        <main className="mx-auto w-full max-w-[1500px] px-6 py-6">
                  ...
                  {!loading && visibleLeads.map(({ lead, index }) => {
                    ...
                    return (
                      <React.Fragment key={lead.id}>
                        <tr className={cn("border-b border-slate-100 transition-colors duration-150", isFlashed ? "bg-emerald-50/60" : followUp.overdue ? "bg-rose-50/30 hover:bg-rose-50/60" : "hover:bg-slate-50/70")} style={{ boxShadow: `inset 3px 0 0 ${railColor}` }}>
                          <td className="px-2 py-2.5 align-middle"><p className={cn(TX.dense, "text-slate-400")}>{serialNumber}</p></td>
                          <td className="px-2 py-2.5 align-middle">
                            <div className="flex min-w-0 items-center gap-2.5">
                              <button onClick={() => setDrawerLeadId(lead.id)} title="Open lead timeline" className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-full font-mono font-semibold transition-opacity duration-150 hover:opacity-80", TX.dense, tone.bg, tone.text)}>{initials(lead.name)}</button>
                              <div className="min-w-0 flex-1 text-left">
                                <div className="flex min-w-0 items-center gap-1.5">
                                  <button onClick={() => setDrawerLeadId(lead.id)} title="Open lead timeline" className={cn(TX.emph, "flex min-w-0 items-center gap-1 truncate font-mono font-semibold text-slate-800 transition-colors duration-150 hover:text-slate-600")}>
                          ...
                          <td className="px-2 py-2.5 align-middle">
                            <div className="flex min-w-0 items-center gap-1.5">
                              <input value={draft} onChange={(e) => updateCallDraft(lead.id, e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); submitCallLog(lead.id); } }} placeholder="फोनमा के कुरा भयो?" disabled={isPending} className={cn(INPUT_CLS, TX.dense, "min-w-0 flex-1 w-full")} />
                              <button onClick={() => submitCallLog(lead.id)} disabled={isPending || !draft.trim()} title="Save call log" className={cn(BTN_PRIMARY_CLS, "h-8 w-8 shrink-0 bg-[var(--c-accent)] hover:bg-[var(--c-accent)]/90")}>{isPending ? <Loader2 size={ICON.sm} className="animate-spin" /> : <MessageSquarePlus size={ICON.sm} />}</button>
                            </div>
                            <button onClick={() => setDrawerLeadId(lead.id)} className="mt-1 flex w-full min-w-0 items-start gap-1 text-left transition-colors duration-150 hover:opacity-80" title="View all notes">
                              {latestNote ? <p className={cn(TX.micro, "truncate text-slate-500")}><span className="font-medium text-slate-600">{noteCount === 1 ? "1 note" : `${noteCount} notes`}:</span> {latestNote.note}</p> : <p className={cn(TX.micro, "text-slate-400")}>No notes yet — click to add</p>}
                            </button>
                          </td>
                          <td className="px-2 py-2.5 align-middle"><ChipSelect value={safeStatus} options={STATUS_META} onChange={(v) => updateLeadStatus(lead.id, v)} disabled={isPending} /></td>
                          <td className="px-2 py-2.5 align-middle"><ChipSelect value={safePriority} options={PRIORITY_META} onChange={(v) => updateLeadPriority(lead.id, v)} disabled={isPending} /></td>
                          <td className="px-2 py-2.5 align-middle"><ChipSelect value={lead.learning_mode ?? "online"} options={MODE_META} onChange={(v) => updateLearningMode(lead.id, v)} disabled={isPending} /></td>
                          <td className="px-2 py-2.5 align-middle">
                            <div className="flex min-w-0 items-center gap-1.5">
                              <input type="date" value={toDateInputValue(lead.follow_up_date)} onChange={(e) => updateFollowUpDate(lead.id, e.target.value)} disabled={isPending} className={cn(INPUT_CLS, TX.dense, "min-w-0 flex-1 w-full", followUp.overdue && "border-rose-300 text-rose-700")} />
                              {lead.follow_up_date && <button onClick={() => updateFollowUpDate(lead.id, "")} disabled={isPending} title="Clear follow-up date" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-control)] text-slate-300 transition-colors duration-150 hover:bg-rose-50 hover:text-rose-500 disabled:opacity-50"><X size={ICON.sm} /></button>}
                            </div>
          ...
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
            <p className={cn(TX.dense, "font-mono text-slate-500")}>Showing {rangeStart.toLocaleString()}–{rangeEnd.toLocaleString()} of {totalCount.toLocaleString()} leads</p>
            <div className="flex items-center gap-1.5">
              <button onClick={() => goToPage(page - 1)} disabled={page <= 1 || loading} className={cn(BTN_GHOST_CLS, TX.dense, "h-8 px-2.5")}><ChevronLeft size={ICON.sm} /> Previous</button>
              {pageNumbers.map((p, i) => p === "ellipsis" ? <span key={`e-${i}`} className={cn(TX.dense, "px-1 text-slate-400")}>…</span> : <button key={p} onClick={() => goToPage(p)} disabled={loading} className={cn("flex h-8 min-w-[32px] items-center justify-center rounded-[var(--radius-control)] border font-medium transition-colors duration-150 disabled:cursor-not-allowed", TX.dense, p === page ? "border-[var(--c-blue)] bg-[var(--c-blue)] text-white" : "border-slate-300 text-slate-600 hover:border-[var(--c-blue)]/40 hover:text-[var(--c-blue)]")}>{p}</button>)}
              <button onClick={() => goToPage(page + 1)} disabled={page >= totalPages || loading} className={cn(BTN_GHOST_CLS, TX.dense, "h-8 px-2.5")}>Next <ChevronRight size={ICON.sm} /></button>
            </div>
          </div>
          <p className={cn(TX.micro, "mt-3 text-center font-mono text-slate-400")}>Every edit re-syncs <span className="text-slate-500">updated_at</span> automatically.</p>
        </main>
      </div>
      ...
      <footer className="crm-shell-header shrink-0 border-t border-slate-200 bg-[var(--c-navy)]">
        <div className={cn(TX.dense, "mx-auto flex max-w-[1500px] flex-wrap items-center justify-center gap-x-8 gap-y-2 px-6 py-2.5 text-slate-200")}>
          <span className="flex items-center gap-1.5"><BookOpen size={ICON.sm} className="text-violet-300" /> {courses.length} courses</span>
          <span className="hidden h-3.5 w-px bg-white/15 sm:block" />
          <span className="flex items-center gap-1.5"><UserPlus size={ICON.sm} className="text-emerald-300" /> {stats.newLeads} new leads</span>
          <span className="hidden h-3.5 w-px bg-white/15 sm:block" />
          <span className="flex items-center gap-1.5"><PhoneCall size={ICON.sm} className="text-sky-300" /> {stats.contactedToday} contacted today</span>
          <span className="hidden h-3.5 w-px bg-white/15 sm:block" />
          <span className="flex items-center gap-1.5"><CalendarClock size={ICON.sm} className="text-amber-300" /> {stats.followupsToday} follow-ups today</span>
          <span className="hidden h-3.5 w-px bg-white/15 sm:block" />
