          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-slate-50 p-4 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 transition-colors" 
            ...
            <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-xl shadow-lg shadow-indigo-600/30 transition-all mt-4">
              Unlock Dashboard
            </button>
          </form>
        </div>
...
function OrdersManager({ data, refresh }: { data: Order[], refresh: () => void }) {
  ...
  const filteredData = data.filter((o: Order) => {
    ...
    const matchesSearch = (o.full_name && o.full_name.toLowerCase().includes(s)) ||
                          ...
                          (o.tutor_name && o.tutor_name.toLowerCase().includes(s));
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="flex gap-4">
        ...
        <select 
          className="bg-white border border-slate-200 rounded-2xl px-4 py-3 text-sm font-bold text-slate-700 outline-none shadow-sm cursor-pointer" 
          value={statusFilter} 
              ...
              <tr key={order.id} className="border-b border-slate-50 hover:bg-slate-50/50 cursor-pointer" onClick={() => setSelectedOrder(order)}>
                ...
                <td className="p-6" onClick={(e) => e.stopPropagation()}>
                  <div className="relative inline-block w-36">
                    <select 
                      ...
                      className={`appearance-none w-full px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider outline-none cursor-pointer border border-transparent hover:border-slate-300 transition-all ${statusColors[order.status] || 'bg-slate-100 text-slate-700'}`}
                    >
                      <option value="pending">PENDING</option>
                      <option value="verified">VERIFIED</option>
                      <option value="failed">FAILED</option>
                ...
                <td className="p-6 text-right" onClick={(e) => e.stopPropagation()}>
                  <div className="flex justify-end items-center gap-4">
                    <button onClick={() => handleDelete(order.id)} className="p-2 text-red-600 bg-red-50 rounded-xl hover:bg-red-100 transition-colors" title="Delete Order">
                      <Trash2 size={16}/>
                    </button>
...
function TutorsManager({ data, refresh }: { data: Tutor[], refresh: () => void }) {
  ...
  const filteredData = data.filter((t: Tutor) => {
    ...
    return (
      ...
      (t.subject && t.subject.some(sub => sub.toLowerCase().includes(s)))
    );
  });
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="relative">
        ...
        <input 
          type="text" 
          placeholder="Search by Tutor Name, Location, or Subjects..." 
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className="w-full pl-12 pr-4 py-4 bg-white rounded-2xl border border-slate-200 shadow-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all font-medium text-slate-700"
        />
      ...
      <div className="bg-white rounded-[30px] shadow-xl border border-slate-100 overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs font-black text-slate-400 uppercase tracking-widest">
              <th className="p-6">Tutor Info</th>
            ...
            {filteredData.map(tutor => (
              <tr key={tutor.id} className="border-b border-slate-50 hover:bg-slate-50/50 cursor-pointer" onClick={() => setSelectedTutor(tutor)}>
                ...
                <td className="p-6 text-right">
                  <div className="flex justify-end items-center gap-4">
                    <button onClick={(e) => handleDeleteTutor(tutor.id, e)} className="p-2 text-red-600 bg-red-50 rounded-xl hover:bg-red-100 transition-colors" title="Delete Tutor">
                      <Trash2 size={16}/>
                    </button>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="relative">
        ...
        <input 
          type="text" 
          placeholder="Search by subject, location, or contact name..." 
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className="w-full pl-12 pr-4 py-4 bg-white rounded-2xl border border-slate-200 shadow-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 transition-all font-medium text-slate-700"
        />
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs font-black text-slate-400 uppercase tracking-widest">
              ...
              <th className="p-6">Subject & Location</th>
              <th className="p-6">Urgent</th>
              <th className="p-6">Status</th>
              <th className="p-6 text-right">Actions</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.map((item: Vacancy) => (
              <tr key={item.id} onClick={() => setViewVacancy(item)} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors cursor-pointer">
                ...
                <td className="p-6">
                  <ToggleSwitch checked={!!item.urgent} onChange={() => handleToggleUrgent(item)} label={item.urgent ? 'URGENT' : 'NORMAL'} activeColor="bg-red-500" activeText="text-red-600" />
                </td>
                <td className="p-6">
                  <ToggleSwitch checked={item.status} onChange={() => handleToggleStatus(item)} label={item.status ? 'OPEN' : 'CLOSED'} />
                </td>
                <td className="p-6 text-right">
            ...
            {filteredData.length === 0 && (
               <tr><td colSpan={5} className="p-10 text-center text-slate-500">No vacancies found.</td></tr>
            )}
          </tbody>
        </table>
      </div>

      {/* View Detail Modal (Application Details) */}
      {viewVacancy && (
          ...
          <div className="bg-white rounded-[30px] shadow-2xl max-w-2xl w-full relative overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 border-b border-slate-100 bg-slate-50 mt-4">
              ...
              <p className="text-sm font-bold text-slate-500 mt-1 flex items-center gap-2">
                <MapPin size={14}/> {viewVacancy.location}
              </p>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Contact Name</p>
                  <p className="font-bold text-slate-800">{viewVacancy.contact_name || '-'}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Phone</p>
                  <p className="font-bold text-slate-800">{viewVacancy.contact_number || '-'}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Salary Range</p>
                  <p className="font-bold text-slate-800 text-green-600">{viewVacancy.salary_range || '-'}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Class Time</p>
                  <p className="font-bold text-slate-800">{viewVacancy.class_time || '-'}</p>
                </div>
              </div>
                  ...
                  <div className="space-y-3">
                    {vacancyApps.map((app: any) => {
                      ...
                      <div key={app.id} className={`flex justify-between items-center p-4 rounded-2xl border ${app.status === 'accepted' ? 'bg-green-50 border-green-200' : app.status === 'rejected' ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200 shadow-sm'}`}>
                        <div>
                          <a href={tutorLink} target="_blank" className="font-bold text-base text-indigo-600 hover:underline flex items-center gap-1">
                            {app.applicant_name} <ExternalLink size={14}/>
                          </a>
                          <p className="text-sm text-slate-500 mt-0.5">{app.applicant_phone} • {app.applicant_email}</p>
                        </div>
                        <div className="text-right">
                          <span className={`inline-block px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider ${app.status === 'accepted' ? 'bg-green-200 text-green-800' : app.status === 'rejected' ? 'bg-red-200 text-red-800' : 'bg-orange-100 text-orange-700'}`}>
                            {app.status}
                          </span>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-[30px] shadow-2xl max-w-2xl w-full relative overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Subject</label>
                  <input type="text" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm transition-colors" value={editingData.subject || ''} onChange={e => setEditingData({...editingData, subject: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Location</label>
                  <input type="text" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm transition-colors" value={editingData.location || ''} onChange={e => setEditingData({...editingData, location: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Contact Name</label>
                  <input type="text" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm transition-colors" value={editingData.contact_name || ''} onChange={e => setEditingData({...editingData, contact_name: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Contact Phone</label>
                  <input type="text" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm transition-colors" value={editingData.contact_number || ''} onChange={e => setEditingData({...editingData, contact_number: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Salary Range</label>
                  <input type="text" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm transition-colors" value={editingData.salary_range || ''} onChange={e => setEditingData({...editingData, salary_range: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Class Time</label>
                  <input type="text" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm transition-colors" value={editingData.class_time || ''} onChange={e => setEditingData({...editingData, class_time: e.target.value})} />
                </div>
              </div>
              <div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="bg-white rounded-[30px] shadow-xl border border-slate-100 overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs font-black text-slate-400 uppercase tracking-widest">
              <th className="p-6">Applicant (Tutor)</th>
              <th className="p-6">Contact Info</th>
              <th className="p-6">Applied For</th>
              <th className="p-6">Vacancy Poster (Student)</th>
              <th className="p-6">Student Response</th>
          ...
          <tbody>
            {data.map((item: VacancyApplication) => {
              ...
              <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                <td className="p-6">
                  <a href={tutorLink} target="_blank" className="font-bold text-indigo-600 hover:underline text-base flex items-center gap-1">
                    {item.applicant_name} <ExternalLink size={14}/>
                  </a>
                </td>
                <td className="p-6">
                   <p className="text-sm font-bold text-slate-700">{item.applicant_phone}</p>
                   <p className="text-xs font-medium text-slate-400 truncate max-w-[150px]">{item.applicant_email}</p>
                </td>
                <td className="p-6">
                  <span onClick={() => window.open(`/vacancies/${item.vacancy_id}`, '_blank')} className="cursor-pointer text-sm text-slate-700 font-bold hover:text-indigo-600 hover:underline transition-colors flex items-center gap-1">
                    {item.vacancies?.subject}
                  </span>
                </td>
                <td className="p-6">
                   <p className="text-sm font-bold text-slate-700">{item.vacancies?.contact_name || 'N/A'}</p>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative inline-block w-36">
                      <select 
                        ...
                        className={`appearance-none w-full px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider outline-none cursor-pointer border border-transparent hover:border-slate-300 transition-all ${statusColors[item.status] || 'bg-slate-100 text-slate-700'}`}
                      >
                        <option value="pending">PENDING</option>
                    ...
                    {item.user_id && (
                      <button onClick={() => onOpenChat(item.user_id as any)} className="p-2 text-indigo-600 bg-indigo-50 rounded-xl hover:bg-indigo-100 transition-colors" title="Chat with Tutor">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs font-black text-slate-400 uppercase tracking-widest">
              ...
              <th className="p-6">Applied To (Tutor)</th>
              <th className="p-6">Status</th>
              <th className="p-6 text-right">Actions</th>
            </tr>
          </thead>
          <tbody>
            {data.map((item: StudentRequest) => (
              <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50/50 cursor-pointer transition-colors" onClick={() => setViewMessage(item.message)}>
                <td className="p-6">
                  <p 
                    className="font-bold text-indigo-600 hover:underline text-lg transition-colors inline-block" 
                    onClick={(e) => { e.stopPropagation(); if (item.user_id) onOpenChat(item.user_id); }}
                ...
                <td className="p-6">
                  {item.tutors ? (
                    <div>
                      <p 
                        className="font-bold text-indigo-600 hover:underline text-base transition-colors inline-block"
                        onClick={(e) => { e.stopPropagation(); if (item.tutors?.user_id) onOpenChat(item.tutors.user_id); }}
            ...
            {data.length === 0 && (
               <tr><td colSpan={4} className="p-10 text-center text-slate-500">No requests found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {viewMessage && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm" onClick={() => setViewMessage(null)}>
          <div className="bg-white rounded-[30px] shadow-2xl max-w-lg w-full relative p-8" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 text-slate-700 font-medium whitespace-pre-wrap leading-relaxed">
              {viewMessage}
            </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => setSelectedCourse(null)} className="p-3 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm">
              <ArrowLeft size={20} className="text-slate-600" />
            </button>
            <div>
              <h2 className="text-2xl font-black text-slate-900">{selectedCourse.title}</h2>
              <p className="text-sm font-medium text-slate-500">Managing Enrollments</p>
            </div>
          </div>

          <div className="flex gap-4 items-center">
             <select 
               className="bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none shadow-sm cursor-pointer" 
               value={dateSort} 
            ...
            <tbody>
              {courseEnrollments.map(enr => (
                <tr key={enr.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  ...
                  <td className="p-6">
                    <p className="text-sm text-slate-800 font-medium">{enr.email}</p>
                    <p className="text-xs text-slate-500 mt-1">WA: {enr.whatsapp_number}</p>
                  </td>
                  <td className="p-6 text-sm text-slate-500 font-medium">
                    {new Date(enr.created_at).toLocaleDateString()}
                  </td>
                  <td className="p-6 text-sm text-slate-600 max-w-[150px] truncate" title={enr.remarks}>
                    {enr.remarks || '-'}
                  </td>
                  <td className="p-6">
                    <ToggleSwitch 
                      ...
                      label={enr.confirmed ? 'Confirmed' : 'Pending'} 
                      activeColor="bg-green-500" 
                      activeText="text-green-600"
                    />
                  </td>
                  <td className="p-6 text-right">
                    <button onClick={async () => { if(confirm('Remove this enrollment?')) { const {error} = await supabase.from('enrollments').delete().eq('id', enr.id); if(error) alert(error.message); else refresh(); } }} className="p-2 text-red-600 bg-red-50 rounded-xl hover:bg-red-100 transition-colors" title="Delete Enrollment">
                      <Trash2 size={16}/>
                    </button>
                  </td>
                </tr>
              ...
              {courseEnrollments.length === 0 && (
                <tr><td colSpan={6} className="p-10 text-center text-slate-500">No matching enrollments found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Select a course to view and manage its enrollments.</p>
        </div>
      </div>
        ...
        {courses.map(course => {
          ...
          return (
            <div 
              ...
              className="bg-white border border-slate-200 rounded-3xl overflow-hidden hover:shadow-xl hover:border-indigo-300 transition-all cursor-pointer group"
            >
              <div className="h-40 w-full bg-slate-100 relative overflow-hidden">
                {course.cover_pic ? (
                  <img src={course.cover_pic} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-slate-300"><BookOpen size={48} /></div>
                )}
                <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-xl text-xs font-black text-indigo-700 shadow-sm">
                  {bookingCount} Booking{bookingCount !== 1 ? 's' : ''}
                </div>
              </div>
              <div className="p-6">
                <h3 className="font-black text-lg text-slate-900 leading-tight mb-2 group-hover:text-indigo-600 transition-colors line-clamp-2">
                  {course.title}
                </h3>
                <p className="text-sm font-medium text-slate-500 flex items-center gap-2">
                  <CalendarDays size={14} /> {course.start_datetime ? new Date(course.start_datetime).toLocaleDateString() : 'TBA'}
                </p>
              </div>
            </div>
          )
...
function CoursesManager({ data, refresh }: { data: OnlineCourse[], refresh: () => void }) {
  ...
  const saveCourse = async () => {
    ...
    if(payload.id) {
      const {error} = await supabase.from('online-courses').update(payload).eq('id', payload.id);
      if(error) alert(error.message);
    } else {
      const {error} = await supabase.from('online-courses').insert([payload]);
      if(error) alert(error.message);
    }
    setModalOpen(false);
    refresh();
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-3xl font-black text-slate-900">Online Courses</h2>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs font-black text-slate-400 uppercase tracking-widest">
              ...
              <th className="p-6">Pricing & Details</th>
              <th className="p-6">Status</th>
              <th className="p-6 text-right">Actions</th>
            </tr>
          </thead>
              ...
              <tr key={course.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                <td className="p-6 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">Starts: {course.start_datetime ? new Date(course.start_datetime).toLocaleDateString() : 'N/A'}</p>
                  </div>
                </td>
                <td className="p-6">
                   <p className="font-bold text-slate-800">Rs. {course.fee}</p>
                   {course.discount > 0 && <p className="text-xs text-green-600 font-bold">{course.discount}% OFF</p>}
                </td>
                <td className="p-6">
                  <ToggleSwitch checked={course.is_active} onChange={() => toggleCourseStatus(course)} label={course.is_active ? 'Active' : 'Draft'} />
                </td>
                <td className="p-6 text-right">
      ...
      {modalOpen && editingCourse && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm" onClick={() => setModalOpen(false)}>
          <div className="bg-white rounded-[30px] shadow-2xl max-w-md w-full relative overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setModalOpen(false)} className="absolute top-6 right-6 text-slate-400 hover:text-slate-800 z-10"><X/></button>
            <div className="p-6 border-b border-slate-100 bg-slate-50 mt-4">
              <h3 className="text-2xl font-black text-slate-900">Edit Course Config</h3>
              <p className="text-sm text-slate-500 font-medium mt-1">{editingCourse.title}</p>
            </div>

            <div className="p-6 space-y-4">
               <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Fee (Rs)</label>
                  <input type="number" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm" value={editingCourse.fee || ''} onChange={e => setEditingCourse({...editingCourse, fee: e.target.value as any})} />
               </div>
               <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Discount (%)</label>
                  <input type="number" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm" value={editingCourse.discount || ''} onChange={e => setEditingCourse({...editingCourse, discount: e.target.value as any})} />
               </div>
               <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Start Date/Time</label>
                  <input type="datetime-local" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm" value={editingCourse.start_datetime ? new Date(new Date(editingCourse.start_datetime).getTime() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0,16) : ''} onChange={e => setEditingCourse({...editingCourse, start_datetime: new Date(e.target.value).toISOString()})} />
               </div>
            </div>
...
function CertificatesManager({ data, syllabi, refresh }: { data: Certificate[], syllabi: any[], refresh: () => void }) {
  ...
  const openCreateModal = () => {
    setForm({
      id: null, name: '', email: '', syllabus_name: '',
      issue_date: new Date().toISOString().split('T')[0],
      certificate_code: '', existing_image: '', file: null
    });
    setModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (!form.id && !form.file) {
      alert('You must upload an image when creating a new certificate.');
      return;
    }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={openCreateModal} className="bg-indigo-600 text-white px-5 py-3 rounded-2xl font-bold flex items-center gap-2 shadow-lg shadow-indigo-600/20 hover:bg-indigo-700 transition-all">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100 text-xs font-black text-slate-400 uppercase tracking-widest">
              ...
              <th className="p-6">Issue Date & Code</th>
              <th className="p-6 text-right">Actions</th>
            </tr>
          </thead>
          <tbody>
            {data.map(cert => (
              <tr key={cert.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                ...
                <td className="p-6">
                   <p className="font-bold text-slate-800">{cert.syllabus_name}</p>
                </td>
                <td className="p-6">
            ...
            {data.length === 0 && (
               <tr><td colSpan={4} className="p-10 text-center text-slate-500">No certificates generated yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm" onClick={() => !uploading && setModalOpen(false)}>
          <div className="bg-white rounded-[30px] shadow-2xl max-w-lg w-full relative overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            <button onClick={() => setModalOpen(false)} disabled={uploading} className="absolute top-6 right-6 text-slate-400 hover:text-slate-800 z-10"><X/></button>
            <div className="p-6 border-b border-slate-100 bg-slate-50 mt-4">
              <h3 className="text-2xl font-black text-slate-900">{form.id ? 'Edit Record' : 'Generate Certificate'}</h3>
              <p className="text-sm text-slate-500 font-medium mt-1">Updates the secure verified registry directly.</p>
            </div>

            <div className="p-6 space-y-4">
               <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Student Full Name</label>
                  <input type="text" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
               </div>
               <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Student Email Address</label>
                  <input type="email" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm" value={form.email} onChange={e => setForm({...form, email: e.target.value})} />
               </div>
               <div>
                  ...
                  <datalist id="syllabi-options">
                    {syllabi.map(s => <option key={s.id} value={s.name} />)}
                  </datalist>
               </div>
               <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase mb-1 ml-1">Completion/Issue Date</label>
                  <input type="date" className="w-full bg-slate-50 p-3.5 rounded-xl outline-none border border-slate-200 focus:border-indigo-500 font-bold text-slate-800 text-sm" value={form.issue_date} onChange={e => setForm({...form, issue_date: e.target.value})} />
               </div>
               <div>
                  ...
                  <label className="w-full flex items-center justify-center gap-2 bg-indigo-50 text-indigo-600 p-4 rounded-xl border border-dashed border-indigo-200 cursor-pointer hover:bg-indigo-100 transition-colors">
                     ...
                     <input type="file" accept="image/png, image/jpeg" className="hidden" onChange={e => {
                       if (e.target.files && e.target.files[0]) setForm({...form, file: e.target.files[0]});
                     }} />
                  </label>
               </div>
            </div>
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3 shrink-0">
              <button onClick={() => setModalOpen(false)} disabled={uploading} className="px-6 py-3 font-bold text-slate-500 hover:text-slate-800 disabled:opacity-50">Cancel</button>
              <button onClick={handleSave} disabled={uploading} className="px-8 py-3 rounded-2xl font-black bg-indigo-600 text-white shadow-lg shadow-indigo-600/20 hover:bg-indigo-700 transition-colors disabled:opacity-50 flex items-center gap-2">
                {uploading ? <Loader2 size={16} className="animate-spin" /> : form.id ? 'Save Changes' : 'Create Record'}
              </button>
            </div>
          </div>
        </div>
  ...
  useEffect(() => {
    const fetchChat = async () => {
      ...
      try {
         ...
         console.warn("is_read column missing or update failed");
      }
    };
    fetchChat();
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm pt-20 pb-10">
      <div className="bg-white rounded-[30px] shadow-2xl max-w-4xl w-full flex flex-col h-[85vh] max-h-[850px] overflow-hidden">
        ...
        <div 
          ...
          className="p-6 border-b bg-slate-50 flex justify-between items-center shrink-0 cursor-pointer hover:bg-slate-100 transition-colors select-none group"
        >
          <div>
            <h3 className="text-xl font-black text-slate-900 flex items-center gap-3">
              {displayName}
              <span className={`text-[10px] uppercase tracking-widest px-2.5 py-1 rounded-md font-bold ${role === 'Tutor' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                {role}
              </span>
            </h3>
            <p className="text-sm text-slate-500 font-medium mt-1">
              {displayPhone} • <span className="text-indigo-500 group-hover:underline">{showDetails ? 'Switch to Chat' : 'View Activity Details'}</span>
            </p>
          </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                      <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-100 text-xs text-slate-500 uppercase">
                          <tr><th className="p-4">Subject & Location</th><th className="p-4">Salary</th><th className="p-4">Student Status</th></tr>
                        </thead>
                ...
                <div>
                  ...
                  {tutorIncomingReqs.length === 0 ? <p className="text-sm text-slate-500">No requests found.</p> : (
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                      <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-100 text-xs text-slate-500 uppercase">
                          <tr><th className="p-4">Student Name</th><th className="p-4">Contact</th><th className="p-4">Status</th></tr>
                        </thead>
                        <tbody className="text-sm font-medium">
                ...
                <div>
                  ...
                  {studentVacs.length === 0 ? <p className="text-sm text-slate-500">No vacancies posted.</p> : (
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                      <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-100 text-xs text-slate-500 uppercase">
                          <tr><th className="p-4">Date</th><th className="p-4">Subject</th><th className="p-4">Total Applicants</th></tr>
                        </thead>
                        <tbody className="text-sm font-medium">
                                        ...
                                        <div className="pl-8 space-y-2">
                                          {vacApps.map((a: VacancyApplication) => (
                                            <div key={a.id} className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200">
                                              <div>
                                                <p className="text-sm font-bold text-slate-800">{a.applicant_name}</p>
                                                <p className="text-xs text-slate-500">{a.applicant_phone}</p>
                                              </div>
                ...
                <div>
                  ...
                  {studentOutgoingReqs.length === 0 ? <p className="text-sm text-slate-500">No requests made.</p> : (
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                      <table className="w-full text-left">
                        <thead className="bg-slate-50 border-b border-slate-100 text-xs text-slate-500 uppercase">
                          <tr><th className="p-4">Tutor Applied</th><th className="p-4">Tutor Contact</th><th className="p-4">Status</th></tr>
                        </thead>
                        <tbody className="text-sm font-medium">
            ...
            <div className="flex-1 p-6 overflow-y-auto bg-white flex flex-col gap-4">
              ...
              {chatMessages.map(m => (
                <div key={m.id} className={`flex group items-center ${m.sender_role === 'admin' ? 'justify-end' : 'justify-start'}`}>
                  {m.sender_role === 'admin' && (
                    <button onClick={() => deleteMessage(m.id)} className="opacity-0 group-hover:opacity-100 p-2 text-red-500 hover:bg-red-50 rounded-full mr-2 transition-all"><Trash2 size={16}/></button>
                  )}
                  <div className={`max-w-[75%] px-5 py-3.5 rounded-3xl font-medium text-[15px] break-words whitespace-pre-wrap ${m.sender_role === 'admin' ? 'bg-indigo-600 text-white rounded-tr-sm' : 'bg-slate-100 text-slate-800 rounded-tl-sm'}`}>
                    {m.content}
                  </div>
                  {m.sender_role === 'user' && (
                    <button onClick={() => deleteMessage(m.id)} className="opacity-0 group-hover:opacity-100 p-2 text-red-500 hover:bg-red-50 rounded-full ml-2 transition-all"><Trash2 size={16}/></button>
                  )}
                </div>
              ))}
