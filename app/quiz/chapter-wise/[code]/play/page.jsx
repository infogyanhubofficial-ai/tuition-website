...
function QuizPlayer() {
  ...
  const chapterCode = params.code || 'ACiE0101';
  ...
  useEffect(() => {
    setTimeout(() => {
      const dummyQuestions = [
        ...
        {
          id: 'q2',
          question: `Which type of cement is recommended for underwater construction where speed is critical?`,
          option_a: 'Ordinary Portland Cement',
          option_b: 'Quick Setting Cement',
          option_c: 'Sulphate Resisting Cement',
          option_d: 'Low Heat Cement',
        },
        {
          id: 'q3',
 ...
 const handleSubmit = () => {
    ...
    // Here you would normally push to Supabase: await supabase.from('quiz_attempts').insert(...)
    
    // Simulate network request to save attempt
    setTimeout(() => {
      // 1. We removed the annoying alert()
      // 2. We are now routing directly to the Results Page we built!
      router.push(`/quiz/results/dummy-attempt-123`);
    }, 1000);
  };
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[radial-gradient(circle_at_top,_#eff6ff,_#f8fafc_40%)]">
        <div className="w-full max-w-2xl px-4">
          <div className="mx-auto h-10 w-10 animate-spin rounded-full border-4 border-slate-200 border-t-blue-600 mb-6"></div>
          <h2 className="text-center text-xl font-bold text-slate-800 animate-pulse mb-8">Preparing your practice session...</h2>
          {/* Skeleton UI */}
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-4xl mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <span className="inline-block px-3 py-1 bg-slate-200 text-slate-700 rounded-lg text-xs font-bold uppercase tracking-widest mb-2 shadow-sm">
            {chapterCode}
          </span>
          <h2 className="text-2xl font-black text-slate-900 leading-tight">Chapter Practice</h2>
        </div>
        
        <div className="flex items-center gap-3">
          {/* Difficulty Badge */}
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-bold shadow-sm ring-1 
            ${difficulty === 'easy' ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 
              difficulty === 'hard' ? 'bg-red-50 text-red-700 ring-red-200' : 
              'bg-amber-50 text-amber-700 ring-amber-200'}`}>
            <span className="text-lg">
              {difficulty === 'easy' ? '🟢' : difficulty === 'hard' ? '🔴' : '🟡'}
            </span>
            <span className="uppercase tracking-wide">{difficulty} Mode</span>
          </div>
      ...
      <div className="w-full max-w-4xl bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex flex-wrap gap-2">
        {questions.map((q, idx) => {
          ...
          let blockClass = "bg-white border-slate-200 text-slate-500 hover:bg-slate-50"; // default unvisited
          if (isRev) blockClass = "bg-amber-100 border-amber-300 text-amber-800"; // yellow for review
          else if (isAns) blockClass = "bg-emerald-100 border-emerald-300 text-emerald-800"; // green for answered
          else if (isVis) blockClass = "bg-slate-100 border-slate-300 text-slate-600"; // gray for visited

          return (
            <button
              key={q.id}
              onClick={() => setCurrentIndex(idx)}
              className={`w-10 h-10 flex items-center justify-center rounded-lg border-2 text-sm font-bold transition-all ${blockClass} ${isCur ? 'ring-2 ring-offset-2 ring-blue-500' : ''}`}
            >
              {idx + 1}
            </button>
          );
                ...
                return (
                  ...
                  >
                    <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl mr-5 font-black text-lg transition-colors ${
                      isSelected ? 'bg-blue-600 text-white shadow-inner' : 'bg-slate-100 text-slate-400 group-hover:bg-blue-100 group-hover:text-blue-600'
                    }`}>
                      {opt}
                    </span>
        ...
        <div className="sticky bottom-0 bg-white/95 backdrop-blur-md border-t border-slate-200 p-4 md:px-10 md:py-6 flex items-center justify-between rounded-b-[28px] z-10">
          ...
          {!isLastQuestion ? (
            ...
            <button
              ...
              className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-10 py-3 rounded-xl font-bold shadow-lg transition-all duration-200 hover:shadow-xl hover:-translate-y-[1px] hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 flex items-center gap-2"
            >
              {status === 'submitting' ? (
      ...
      <div className="mt-8 text-center text-sm font-medium text-slate-400 flex flex-col items-center gap-2">
        <p>Keyboard shortcuts: Use <kbd className="bg-slate-200 px-1.5 py-0.5 rounded text-slate-700">A</kbd> <kbd className="bg-slate-200 px-1.5 py-0.5 rounded text-slate-700">B</kbd> <kbd className="bg-slate-200 px-1.5 py-0.5 rounded text-slate-700">C</kbd> <kbd className="bg-slate-200 px-1.5 py-0.5 rounded text-slate-700">D</kbd> to select, <kbd className="bg-slate-200 px-1.5 py-0.5 rounded text-slate-700">Enter</kbd> for next.</p>
        <p className="flex items-center gap-1 text-blue-500/80">
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
          </svg>
          Your performance will be AI-analyzed after submission
        </p>
      </div>
...
export default function Page() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[radial-gradient(circle_at_top,_#eff6ff,_#f8fafc_40%)] flex items-center justify-center">Loading...</div>}>
      <QuizPlayer />
    </Suspense>
  );
}